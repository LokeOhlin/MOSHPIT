...
for idx, f in enumerate(args.f):
    ...
    dadt_c = np.abs(hfile['dadt'][:,:isilicone])
    dadt_s = np.abs(hfile['dadt'][:,isilicone:])
    ...
    for i in range(len(idxs)):
        ...
        ax.plot(abin_c, dadt_c[idxs[i],:], c = color, label = r'$R$ = {:.2e} pc'.format(rad[idxs[i]]/cgs.pc))
        ax.plot(abin_c, dadt_s[idxs[i],:], c = color, ls = '--')
