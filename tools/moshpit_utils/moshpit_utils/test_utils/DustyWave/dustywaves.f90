!-----------------------------------------------------------------------
! Subroutine to compute exact solution for the DUSTYWAVE problem
! from Laibe & Price (2011), MNRAS xxx, xxx
! http://arXiv.org/abs/1106.1736
!
! Written by Daniel Price (2011), daniel.price@monash.edu
! This code is in the public domain, and may be freely copied,
! modified and distributed provided due attribution is given.
!
!-----------------------------------------------------------------------
module dustywaves
  implicit none
...
!
! Input:
!  time     : time at which to compute the wave solution
!  ampl     : amplitude of the velocity and density perturbations
!             (for this routine assumed to be the same, though this could be relaxed)
!  cs       : sound speed in the gas
!  Kdragin  : drag coefficient between the two fluids
!  lambda   : wavelength of the solution
!  x0       : starting position in box
!  rhogeq   : equilibrium gas density
!  rhodeq   : equilibrium dust density
!  xplot(:) : input array containing spatial positions to compute solution at
! 
! Output:
!  vgaso(:)    : gas velocity (array of values corresponding to each xplot)
!  vdusto(:)   : dust velocity (array of values corresponding to each xplot)
!  rhogaso(:)  : gas density (array of values corresponding to each xplot)
!  rhodusto(:) : dust density (array of values corresponding to each xplot)
!  ierr        : ierr /= 0 is returned if there are fatal errors in the input parameters
!
! Dependencies:
!  cubicsolve_complex : a routine returning the three complex roots to a cubic equation
!
!-----------------------------------------------------------------------------------------
subroutine exact_dustywave(time,ampl,cs,Kdragin,lambda,x0,rhogeq,rhodeq, xplot,vgaso,vdusto,rhogaso,rhodusto,ierr)
  use cubic,   only:cubicsolve_complex
  implicit none
  integer :: i
  real, parameter :: pi = 3.1415926536
  real, intent(in)    :: time, ampl, cs, Kdragin, lambda, x0, rhogeq, rhodeq
  real, intent(in), dimension(:) :: xplot
  real, intent(out), dimension(size(xplot)) :: vgaso,vdusto,rhogaso,rhodusto
  integer, intent(out) :: ierr
  real :: rhodsol,rhogsol,vdeq,vgeq,vgsol,vdsol
...
!-------------------------------
  do i=1,size(xplot)
     xk =  2.*pi/lambda*(xplot(i)-x0) - pi ! the -pi is because in the paper we assume v = v0 sin(kx) not v0 cos(kx)
     arg1 = xk - w1r*time
     arg2 = xk - w2r*time
     arg3 = xk - w3r*time
     vgas = vgeq &
